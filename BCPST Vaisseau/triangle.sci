function newgame(pseudo,couleur)
disp(pseudo,couleur)
...
//-----------------------------------------------
scf()
while rep(3) do    //triangle BAC, M est le milieu de BC (A en haut)
  rep=xgetmouse();
  ...
  if rep(3)==122|rep(3)==115 then   // code ascii : "z"->122 , "s"->115
    ...
    elseif rep(3)==122 & Mat(1,2)>2 & Mat(1,2)<54 & Mat(2,2)>2 & Mat(2,2)<54 then k=1;
      ...
      elseif RIP==1&rep(3)==115 then k=1;
       ...
       elseif RIP==2 then messagebox("You''re dead Jack... RIP"), fin=1;
      end
     end
 ...
 xpause(5)
 if fuel<=0 then messagebox(["Vous n''avez plus de carburant, Jack... Vous avez échoué!"," Vous et votre vaisseau allez dériver jusqu''à la fin des temps dans le froid et la solitude de l''univers..."]), fin=1;
 end
end
